...
func (h *Handler) BindWebsite(c *gin.Context) {
	...
	// 触发 website_release_task（仅 active 且 group 模式）
	if website.Status == model.WebsiteStatusActive && website.OriginMode == model.OriginModeGroup {
		h.triggerWebsiteReleaseTask(req.WebsiteID)
	}
